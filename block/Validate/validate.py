...
import hashlib
import os
from flask import Flask, render_template, current_app, request, redirect, url_for, g, json
...
def get_current_hash(encrypted_data):
    conn = sqlite3.connect('../block/logs.db')
    c = conn.cursor()
...
def index():
    if request.method == 'POST':
        ...
        if is_data_present(conn, encrypted_data):
            ...
            print("type real data = ", type(real_data))
